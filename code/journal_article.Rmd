...
llama_base_judgments <- read_csv(here("../data/llama_3.1_70b_base_judgments.csv")) %>%
  mutate(llama_base_score = case_when(llama_base_judgment == "never" ~ 1,
                                llama_base_judgment == "rarely" ~ 2,
                                llama_base_judgment == "sometimes" ~ 3,
                                llama_base_judgment == "about half the time" ~ 4,
                                llama_base_judgment == "often" ~ 5,
                                llama_base_judgment == "almost always" ~ 6,
                                llama_base_judgment == "always" ~ 7))

llama_instruct_judgments <- read_csv(here("../data/llama_3.1_70b_instruct_judgments.csv")) %>%
  mutate(llama_instruct_score = case_when(llama_70b_instruct_judgment == "never" ~ 1,
                                llama_instruct_judgment == "rarely" ~ 2,
                                llama_instruct_judgment == "sometimes" ~ 3,
                                llama_instruct_judgment == "about half the time" ~ 4,
                                llama_instruct_judgment == "often" ~ 5,
                                llama_instruct_judgment == "almost always" ~ 6,
                                llama_instruct_judgment == "always" ~ 7))

llama_base_judgments <- read_csv(here("../data/llama_3.1_70b_base_judgments.csv")) %>%
  mutate(llama_base_score = case_when(llama_70b_base_judgment == "never" ~ 1,
                                llama_base_judgment == "rarely" ~ 2,
                                llama_base_judgment == "sometimes" ~ 3,
                                llama_base_judgment == "about half the time" ~ 4,
                                llama_base_judgment == "often" ~ 5,
...
llama_405b_base_judgments <- read_csv(here("../data/llama_3.1_405b_base_judgments.csv")) %>%
  mutate(llama_405b_base_score = case_when(llama_405b_base_judgment == "never" ~ 1,
                                llama_405b_base_judgment == "rarely" ~ 2,
                                llama_405b_base_judgment == "sometimes" ~ 3,
                                llama_405b_base_judgment == "about half the time" ~ 4,
                                llama_405b_base_judgment == "often" ~ 5,
                                llama_405b_base_judgment == "almost always" ~ 6,
                                llama_405b_base_judgment == "always" ~ 7))

llama_405b_instruct_judgments <- read_csv(here("../data/llama_3.1_405b_instruct_judgments.csv")) %>%
  mutate(llama_405b_instruct_score = case_when(llama_405b_instruct_judgment == "never" ~ 1,
                                llama_405b_instruct_judgment == "rarely" ~ 2,
                                llama_405b_instruct_judgment == "sometimes" ~ 3,
                                llama_405b_instruct_judgment == "about half the time" ~ 4,
                                llama_405b_instruct_judgment == "often" ~ 5,
                                llama_405b_instruct_judgment == "almost always" ~ 6,
                                llama_405b_instruct_judgment == "always" ~ 7))
...
all_judgments <- mean_ratings %>%
  ...
  left_join(llama_base_judgments, by = c("adjective", "noun")) %>%
  left_join(llama_instruct_judgments, by = c("adjective", "noun")) %>%
  left_join(llama_405b_base_judgments, by = c("adjective", "noun")) %>%
  left_join(llama_405b_instruct_judgments, by = c("adjective", "noun")) %>%
...
human_cor <- mean(c(
cor.test(subj_reliability$rating_1, subj_reliability$mean_1, method = "spearman")$estimate,
cor.test(subj_reliability$rating_2, subj_reliability$mean_2, method = "spearman")$estimate,
cor.test(subj_reliability$rating_3, subj_reliability$mean_3, method = "spearman")$estimate,
cor.test(subj_reliability$rating_4, subj_reliability$mean_4, method = "spearman")$estimate,
cor.test(subj_reliability$rating_5, subj_reliability$mean_5, method = "spearman")$estimate,
cor.test(subj_reliability$rating_6, subj_reliability$mean_6, method = "spearman")$estimate))

subj_reliability %>%
  ggplot(aes(x = rating_1, y = mean_typ)) +
  geom_jitter(aes(alpha = 0.2))

model_cors <- data.frame(
  model = c("LDP_w2v", "Wiki_w2v", "BERT", "GPT_3", "Llama 70b Base", "Llama 70b Instruct", "Llama 405b Base", "Llama 405b Instruct", "Human"),
  cor = c(cor.test(all_judgments$mean_typ, all_judgments$ldp_similarity, method = "spearman")$estimate,
          cor.test(all_judgments$mean_typ, all_judgments$wiki_similarity, method = "spearman")$estimate,
          cor.test(all_judgments$mean_typ, all_judgments$bert_prob, method = "spearman")$estimate,
          cor.test(all_judgments$mean_typ, all_judgments$gpt3_score, method = "spearman")$estimate,
          cor.test(all_judgments$mean_typ, all_judgments$llama_base_score, method = "spearman")$estimate,
          cor.test(all_judgments$mean_typ, all_judgments$llama_instruct_score, method = "spearman")$estimate,
          cor.test(all_judgments$mean_typ, all_judgments$llama_405b_base_score, method = "spearman")$estimate,
          cor.test(all_judgments$mean_typ, all_judgments$llama_405b_instruct_score, method = "spearman")$estimate,
          human_cor)
)


cors_plot <- model_cors %>%
  mutate(model = factor(model, levels = c("LDP_w2v", "Wiki_w2v", "BERT", "GPT_3", 
                                          "Llama 70b Base", "Llama 70b Instruct", 
                                          "Llama 405b Base", "Llama 405b Instruct",
                                          "Human"))) %>%
...
ggsave(cors_plot, filename = here("figs/model_cors_plot.png"),  width = 6, height = 4, units = "in", dpi = 300, bg = "transparent")


all_judgments %>%
  ggplot(aes(x = mean_typ, y = gpt3_score)) +
  geom_jitter(width = 0.2, height = 0.2, aes(alpha = 0.1)) +
  geom_smooth(method='lm', formula= y~x)

random_indices <- sample(1:nrow(all_judgments), 10)
all_judgments <- all_judgments %>%
  mutate(pair = paste(adjective, noun, sep = " ")) %>%
  mutate(label_name = NA)
all_judgments$label_name[random_indices] <- all_judgments$pair[random_indices]

all_judgments %>%
  ggplot(aes(x = mean_typ, y = llama_instruct_score, label = label_name)) +
  geom_jitter(aes(alpha = 0.2)) +
  geom_smooth() +
  xlab("Human Judgment (mean)") +
  ylab("Llama Instruct Judgment") +
  geom_label_repel(
    fill = "white",  # Sets the background color to white
    colour = "black" # Sets the text color to black
  )

all_judgments %>%
  ggplot(aes(x = mean_typ, y = llama_base_score, label = label_name)) +
  geom_jitter(aes(alpha = 0.2)) +
  geom_smooth() +
  xlab("Human Judgment (mean)") +
  ylab("Llama Base Judgment") +
  geom_label_repel(
    fill = "white",  # Sets the background color to white
    colour = "black" # Sets the text color to black
  )
...
llama_base_cor <- wtd.cor(all_judgments %>% pull(mean_typ),
                    all_judgments %>% pull(llama_base_score)) %>%
  as_tibble()

llama_instruct_cor <- wtd.cor(all_judgments %>% pull(mean_typ),
                    all_judgments %>% pull(llama_instruct_score)) %>%
  as_tibble()
...
llama_base_cor_estimate <- llama_base_cor %>% pull(correlation)
llama_base_cor_p <- llama_base_cor %>% pull(p.value) %>% printp()

llama_instruct_cor_estimate <- llama_instruct_cor %>% pull(correlation)
llama_instruct_cor_p <- llama_instruct_cor %>% pull(p.value) %>% printp()
...
correct_orders <- high_low_all %>%
  ...
  pivot_longer(cols = c(ldp_similarity, wiki_similarity, bert_prob, gpt3_score, 
                        llama_base_score, llama_instruct_score), 
               names_to = "measure", values_to = "similarity") %>%
  select(-adjective, -mean_typ, -gpt3_judgment, -llama_base_judgment, -llama_instruct_judgment) %>%
...
llama_base_correct <- correct_orders %>%
  filter(measure == "llama_base_score") %>%
  pull(correct)

llama_instruct_correct <- correct_orders %>%
  filter(measure == "llama_instruct_score") %>%
  pull(correct)
...
gpt3_binom <- binom.test(gpt3_correct, pairs_total)$p.value %>%
  printp()
llama_base_binom <- binom.test(llama_base_correct, pairs_total)$p.value %>%
  printp()
llama_instruct_binom <- binom.test(llama_instruct_correct, pairs_total)$p.value %>%
  printp()
```
```{r typ-plots-compute}
...
halves_data <- high_low_all %>%
  select(-gpt3_judgment, -llama_base_judgment) %>%
  ...
  mutate(measure = factor(measure, 
                          levels = c("mean_typ", "ldp_similarity", 
                                     "wiki_similarity", "bert_prob", "gpt3_score", "llama_base_score",
                                     "llama_instruct_score"), 
                          labels = c("Human", "LDP word2vec", 
                                     "Wiki word2vec", "BERT", "GPT-3", "Llama 70b Base",
                                     "Llama 70b Instruct"))) %>%
...
human <- halves_data %>%
  ...
  mutate(wiki_similarity = true_mean*(wiki_max - wiki_min) + wiki_min,
         ...
         llama_base_score = true_mean*1) %>%
  pivot_longer(c(true_mean, wiki_similarity, ldp_similarity, bert_prob, gpt3_score, llama_base_score),
...
means <- halves_data %>%
  ...
  mutate(mean = if_else(measure=="Llama 70b Base", (mean - 1)/6, mean)) %>%
  mutate(mean = if_else(measure=="Llama 70b Instruct", (mean - 1)/6, mean)) %>%
    filter(measure != "Human") 
...
halves_data %>%
  filter(measure %in% c("BERT", "GPT-3", "Llama 70b Base", "Llama 70b Instruct")) %>%
  mutate(score = if_else(measure=="GPT-3", (score - 1)/6, score)) %>%
  mutate(score = if_else(measure=="Llama 70b Base", (score - 1)/6, score)) %>%
  mutate(score = if_else(measure=="Llama 70b Instruct", (score - 1)/6, score)) %>%
  #mutate(score = if_else(measure=="BERT", log(score), score)) %>%
  mutate(measure = factor(measure, 
                          levels = c("BERT", "GPT-3", "Llama 70b Base", "Llama 70b Instruct"))) %>%
  ggplot(aes(x = typicality, y = score, group = noun)) +
  facet_wrap(~measure) +
  geom_point(alpha = .5, position=position_dodge(0.06), color = "light gray") +
  geom_line(alpha = .5, color = "light gray", position=position_dodge(0.06)) +
  geom_line(data=human %>% filter(measure %in% c("BERT", "GPT-3", "Llama 70b Base", "Llama 70b Instruct")),
            aes(x=typicality, y=mean, group=NA), color="black", linetype="dashed") +
  geom_point(data=human %>% filter(measure %in% c("BERT", "GPT-3", "Llama 70b Base", "Llama 70b Instruct")),
             aes(x=typicality, y=mean, group=NA), color="black") +
  geom_line(data=means %>% filter(measure %in% c("BERT", "GPT-3", "Llama 70b Base", "Llama 70b Instruct")),
            aes(x=typicality, y=mean, group=NA), color="steelblue", size=.8, alpha = 0.7) +
  geom_point(data=means %>% filter(measure %in% c("BERT", "GPT-3", "Llama 70b Base", "Llama 70b Instruct")), 
             aes(x=typicality, y=mean, group=NA), color="steelblue") +
